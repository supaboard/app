...
export function AreaChart({ dashboard_uuid, chartData, timeframe, is_public }) {
	...
	useEffect(() => {
		...
		const fetchChartData = async () => {
			...
			let UsageByDay = data.reduce((acc, item) => {
			...
			if (timeframe) {
				UsageByDay = Object.keys(UsageByDay).reduce((acc, key) => {
					...
					if (timestamp >= timestampFrame[0] && timestamp <= timestampFrame[1]) {
			...
			Object.keys(UsageByDay).map((key) => {
				const date = new Date(key)
				days.push(date.toLocaleDateString())
				dataPoints.push(UsageByDay[key])
				series.push({
					data: [UsageByDay[key]],
